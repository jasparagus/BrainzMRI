...
* **Dedicated Likes for Days Mode:** The "Likes for Days" report is a permanent report mode that computes time-weighted listening for cross-service likes (ListenBrainz + Last.fm). It operates on the loaded User History, pre-filtering likes to tracks with 2 or more listens to minimize API traffic. It enriches qualifying tracks with verified track durations from MusicBrainz via `enrichment.fetch_recording_durations()` and caches them persistently in `duration_cache.json`. Results are aggregated across Tracks, Albums, and Artists ranked by `total_duration_ms` (`duration × listens`). Automatic name canonicalization leverages MusicBrainz IDs (`_normalize_names_by_mbid`) and case-insensitive mode matching to unify casing variations across artists and releases. An optional pre-flight prompt (`LikesSyncPromptDialog`) allows the user to sync likes and resolve unmapped tracks before report generation.
* **UI State Management:** The `gui_main.py` controller maintains report modes ("Imported Playlist", "Likes", and "Likes for Days") as permanent entries in the dropdown, not dynamically toggled.
...
### 3.10 Platform-Independent Font Architecture & Display Scaling
* **Platform-Aware Font Family Probe (`_resolve_font_family`):** Font setup dynamically probes available system font families at startup (`Segoe UI` on Windows; `Noto Sans`, `DejaVu Sans`, `Liberation Sans`, `Helvetica` on Linux) to prevent unstyled system default font fallbacks across OSes.
* **Native Tcl Font Registry:** Custom named fonts (`AppFont`, `AppFontBold`, `AppFontItalic`, `AppFontSmall`, `AppFontLarge`, `AppFontLargeBold`) MUST be registered and configured directly in Tcl's native font registry (`self.root.tk.call("font", "create"/"configure", ...)`) rather than left as unlinked Python objects. This prevents Tcl string lookup failures that trigger unstyled fallback to `Arial 15`.
* **Global Named Font Cascading:** Standard Tk named fonts (`TkDefaultFont`, `TkTextFont`, `TkMenuFont`, `TkCaptionFont`, `TkHeadingFont`) and `ttk.Style()` are overridden at startup so standard widgets automatically scale cleanly without adding explicit `font=` calls to individual widget instantiations.
* **Dynamic Dialog Geometries (`minsize` Floors):** Modal dialogs (`ActionConfirmDialog`, `ResolveConfirmDialog`, `ProgressWindow`) MUST use flexible height/width floors (`minsize`) rather than hardcoded fixed `geometry("WxH")` bounds. This prevents text clipping across OSes, display scales, and DPI settings.
* **Treeview Row Height Scaling:** `style.configure("Treeview", rowheight=int(22 * scale))` MUST be explicitly updated whenever TTK fonts or scaling factors change to prevent vertical row collisions.
* **Linear Display Scaling (`config.display_scale`):** Base font sizes use fixed point units (`9pt`, `8pt`, `10pt`) while `tk scaling`, window dimensions (`base_w * scale`, `base_h * scale`), and Treeview `rowheight` scale linearly with `config.display_scale`, preventing compound double-scaling bugs on 4K/High-DPI screens.
...
* **`gui_actions.py`:** Bottom bar. Upstream actions (Like, Resolve, Export) and modal dialogs (`LikesSyncPromptDialog`, `ResolveConfirmDialog`, `ActionConfirmDialog`). Owns the worker threads for these actions.
* **`gui_tableview.py`:** `ttk.Treeview` wrapper. Handles sorting (`mergesort`), regex filtering, and column rendering.
* **`gui_charts.py`:** Native Matplotlib window generation (Entity Trends for Artist/Track/Album, New Music, Genre Treemap, Likes for Days Treemaps (Artists, Albums, Tracks), Album Art Matrix).
