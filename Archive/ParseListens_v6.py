...
def report_top(df, group_col="artist", years=None, by="total_tracks", topn=100):
    ...
    years: None, 2, etc.
    by: "total_duration_hours" or "total_tracks"
    """
    if years:
...
def enrich_report_with_genres(report_df, zip_path, report_name="Artists_In_Library"):
    import os
    base_dir = os.path.dirname(zip_path)
    reports_dir = os.path.join(base_dir, "reports")
    os.makedirs(reports_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    # Save basic reports
    artists_df, meta = report_top(df, group_col="artist", years=3, by="total_tracks", topn=100)
    save_report(artists_df, zip_path, meta=meta)
    
    albums_df, meta = report_top(df, group_col="album", years=3, by="total_tracks", topn=100)
    save_report(albums_df, zip_path, meta=meta)
    
    artist_likes_report = report_artists_with_likes(df, feedback)
    save_report(artist_likes_report, zip_path, report_name="Artists_With_Likes")
