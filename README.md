...
* **Likes for Days:** A time-weighted analysis of your favorite music. Filters cross-platform likes (ListenBrainz + Last.fm) to tracks with 2 or more listens, retrieves verified track durations from MusicBrainz, and compiles Top-N lists aggregated across **Tracks**, **Albums**, and **Artists** ranked by total listening duration (`duration × listens`). Features automatic MBID-based and case-insensitive name canonicalization (e.g. merging casing variants like "Between The Buried And Me" and "Between the Buried and Me") and an optional pre-flight prompt to sync fresh likes and resolve unmapped tracks before generation.
...
* **Likes for Days Treemaps:** Three cascaded squarify treemap windows (Artists using `viridis`, Albums using `inferno`, and Tracks using `plasma`) accessible via the **Show Graph** button. Tiles are sized by total listening duration with overlaid 4-line labels showing entity name, duration/album, listen and like counts, and human-readable total time with high-contrast text strokes.
...
## Presets for filters
* Add a "Preset Report" button that will trigger some common reports (from a secondary modal), such as ("Last Month", "Last Year", 20XX, etc., which will auto-populate the "days ago" or "last listened" filters). Will need to decide on a basic UI (dropdown?) which, when selected, will auto-populate the associated filter(s) relative to the current datetime.

## Heatmaps
* Goal: Visualization for listening density (Hour of Day vs Day of Week).
...
## Listen Deletion [Currently Blocked by API Functionality]
